...
export default function DashboardScreen() {
  ...
  const { exchangeRate, initialBudgetEur, initialBudgetClp, usdExchangeRate } =
  ...
  const totals = useMemo(() => {
    ...
    const initialEur = preTripExpenses.reduce(
      (acc, curr) => acc + curr.amountEur,
      0
    );
    const initialClp = preTripExpenses.reduce(
      (acc, curr) => acc + curr.amountClp,
      0
    );

    const spentEur = tripExpenses.reduce(
      (acc, curr) => acc + curr.amountEur,
      0
    );
    const spentClp = tripExpenses.reduce(
      (acc, curr) => acc + curr.amountClp,
      0
    );
  ...
  }, [expenses, wallets, exchangeRate, usdExchangeRate]);

  const chartData = useMemo(() => {
    const categoryTotals: Record<string, number> = {};
    expenses.forEach((e) => {
      categoryTotals[e.category] =
        (categoryTotals[e.category] || 0) + e.amountEur;
    });
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.summaryRow}>
          <Card style={[styles.summaryCard, { flex: 1, marginRight: 8 }]}>
            ...
            <Typography variant="h3" color="#FF3B30">
              {formatCurrency(totals.spentEur, "EUR")}
            </Typography>
            <Typography variant="caption" color="#999">
              {formatCurrency(totals.spentClp, "CLP")}
            </Typography>
          </Card>
          <Card style={[styles.summaryCard, { flex: 1, marginLeft: 8 }]}>
            ...
            <Typography variant="h3" color="#4CD964">
              {formatCurrency(totals.balanceEur, "EUR")}
            </Typography>
            <Typography variant="caption" color="#999">
              {formatCurrency(totals.balanceClp, "CLP")}
            </Typography>
          </Card>
        </View>
        ...
        {expenses.length > 0 ? (
          <View style={styles.recentListContainer}>
            <ScrollView nestedScrollEnabled style={styles.recentListScroll}>
        ...
        {chartData.length > 0 ? (
          ...
          <Card style={styles.emptyCard}>
            <Typography variant="body" color="#999" align="center">
